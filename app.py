...
# নিশ্চিত করুন যে Render এনভায়রনমেন্টে এই ভেরিয়েবলগুলো সেট করা আছে
...
# FIX: ক্লায়েন্ট শুরু হয়েছে কিনা তা ট্র্যাক করার জন্য গ্লোবাল ফ্ল্যাগ
...
# --- Telethon সংযোগ লজিক (FIXED) ---
@app.before_request
async def connect_telethon():
    """FIX: অ্যাপ্লিকেশন শুরু হওয়ার সময় একবার Telethon ক্লায়েন্ট সংযোগ স্থাপন করে।
       'before_first_request' এর পরিবর্তে 'before_request' ব্যবহার করা হয়েছে।
    """
    global client_initialized
    
    # ক্লায়েন্ট যদি আগে থেকেই শুরু না হয়, তবেই এটি সংযোগ করার চেষ্টা করবে
    if not client_initialized:
        try:
            ...
            # এটি ব্যর্থ হলে, ক্লায়েন্ট_initialized False থাকবে এবং পরের রিকোয়েস্টে আবার চেষ্টা করবে
            print(f"❌ Failed to connect Telethon Client on startup: {e}")

# --- কোর ফাংশন (Async) ---
async def get_entity_info_data(query):
    # বারবার সংযোগ স্থাপনের লজিক সরানো হয়েছে।
    if not client.is_connected():
        return {'status':'error','message':'Telethon client is not connected. Check environment variables (API_ID/HASH/SESSION_STRING).'}
...
def static_files(filename):
    # Render ডিপ্লয়মেন্টের জন্য 'static' ফোল্ডারের অস্তিত্ব নিশ্চিত করুন।
    return send_from_directory('static', filename)


if __name__=='__main__':
    # লোকাল টেস্টিং এর জন্য
    print("⚠️ Local development mode. Use 'uvicorn app:app --host 0.0.0.0 --port 5000' for better local testing of async code.")
    
    # লোকাল রান করার সময় async/await কাজ করতে পারে, কিন্তু production-এ ASGI সার্ভার ব্যবহার করা আবশ্যক।
