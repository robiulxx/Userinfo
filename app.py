...
# --- Telethon async fetch for username ---
async def get_entity_info_telethon(query):
    ...
    try:
        ...
        if is_numeric:
            return None, "Telethon: Skipping numeric ID, Bot API handles this."
        else:
            try:
...
# --- Async Bot API fetch for numeric ID ---
async def get_entity_info_bot_api(user_id):
    if not BOT_TOKEN: return None, "Bot token not set."
...
async def get_entity_hybrid(query):
    if query.isdigit():
        # Numeric ID → always Bot API
        bot_data, bot_err = await get_entity_info_bot_api(int(query))
        if bot_data:
            return {'status':'success','data':bot_data}
        else:
            return {'status':'error','message': bot_err or "Unknown error"}
    else:
        # Username → Telethon
        telethon_data, tele_err = await get_entity_info_telethon(query)
        if tele_err: return {'status':'error','message': tele_err}
        # Optional Bot API fetch for ID
        if telethon_data and BOT_TOKEN:
            bot_data, _ = await get_entity_info_bot_api(telethon_data.get('id'))
            if bot_data:
                telethon_data['bot_api'] = bot_data
...
def api_info():
    ...
    try:
        ...
        import traceback
        print("Error fetching entity:", traceback.format_exc())
